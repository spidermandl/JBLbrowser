...
import com.jbl.browser.tools.ShareTool;
...
public class GameActivity extends Activity{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		else if(requestCode == SHARE_REQUEST && resultCode == ShareActivity.SHARE_DONE){
			int type = data.getIntExtra(StringUtils.REWARD_TYPE, 0);
			if(StringUtils.DEBUG){
				Log.i(TAG, "分享奖励类型：" + type);
			}
			switch (type) {
			case ShareModel.REWARD_TYPE_NULL:
				break;
			case ShareModel.REWARD_TYPE_PT:
				webView.loadUrl(UrlUtils.URL_REWARD_PT);
				break;
			case ShareModel.REWARD_TYPE_CARD:
				webView.loadUrl(UrlUtils.URL_REWARD_CARD);
				break;
			case ShareModel.REWARD_TYPE_ITEM:
				webView.loadUrl(UrlUtils.URL_REWARD_ITEM);
				break;
...
//		webView.getSettings().setLoadWithOverviewMode(true);
	...
	private void initIAppSDK(){
	    ...
	    SDKApi.init(GameActivity.this, 1, IAppPaySDKConfig.APP_ID);
	}
//Desmond end
...
//				});
	...
	private void share(final String data){
		...
		if(data.contains("zxx_playsoundS:!!")){
			parseString = parseString.substring("zxx_playsoundS:!!".length());
		}
		shareModel = null;
		shareModel = ShareTool.getInstance().parseData(this,parseString);
		if(shareModel == null){
			if(StringUtils.DEBUG){
				Log.i(TAG, "分享内容为空");
			}
		...
		if(shareModel.getShareWay() == ShareModel.SHARE_WAY_SINA){
			shareToWeibo();
		}
		else if(shareModel.getShareWay() == ShareModel.SHARE_WAY_SMS){
			ShareTool.getInstance().shareToSMS(this, shareModel);
		}
		else if(shareModel.getShareWay() == ShareModel.SHARE_WAY_MAIL){
			ShareTool.getInstance().shareToMail(this,shareModel);
		}
	...
	private void shareToWeibo(){
		if(shareModel == null){
			if(StringUtils.DEBUG){
				Log.w(TAG, "分享内容shareModel为空");
			}
			Toast.makeText(GameActivity.this, R.string.share_load_fail, 
					Toast.LENGTH_SHORT).show();
			shareModel = null;
		}
		if(shareModel.getShareType() == ShareModel.SHARE_TYPE_UP){
			int level = 0;
			level = shareModel.getShareLevel();
			if(StringUtils.DEBUG){
				Log.v(TAG, "升级分享，当前等级：" + level);
			}
			if(level % 5 != 0){
				return;
			}
		}
		if(ShareTool.getInstance().getToken() != null){ //已经授权且未过期，直接分享
			if(shareModel.getSharePicPath() == null){ //存在图片，下载图片
				ShareTool.getInstance().shareToWeibo(this,shareModel);
			}
			else{
				pd = UserDefinedDialog.getInstance().getProgressDialog(this, 
						getString(R.string.app_name), getString(R.string.wait_download));
				pd.setCancelable(true);
				pd.show();
				String fileName = shareModel.getSharePicPath();
				fileName = fileName.substring(fileName.lastIndexOf("/") + 1);
				if(StringUtils.DEBUG){
					Log.v(TAG, "ShareContent: " + shareModel.getShareContent());
				}
				ShareTool.getInstance().getCardFromUri(UrlUtils.URL_HEAD2 + shareModel.getSharePicPath(),
						fileName, new ShareTool.GetCardCallback() {
							
							@Override
							public void fail(int code) {
								// TODO Auto-generated method stub
								if(StringUtils.DEBUG){
									Log.e(TAG, "下载失败，错误代码：" + code);
								}
								Toast.makeText(GameActivity.this,R.string.download_fail,
										Toast.LENGTH_SHORT).show();
								if(pd != null){
									if(pd.isShowing()){
										pd.dismiss();
							...
							public void done(String path) {
								...
								if(StringUtils.DEBUG){
									Log.v(TAG, "ShareContent: " + shareModel.getShareContent());
									Log.v(TAG, "SharePicName: " + shareModel.getSharePicName());
									Log.v(TAG, "SharePicPath: " + shareModel.getSharePicPath());
								}
								shareModel.setSharePicPath(path);
								ShareTool.getInstance().shareToWeibo(GameActivity.this,shareModel);
								shareType = shareModel.getShareType();
								shareModel = null;
							}
						});
			}
			...
			ShareTool.getInstance().authorize(this, new WeiboAuthListener() {
				...
				public void onWeiboException(WeiboException e) {
					...
					Toast.makeText(GameActivity.this, R.string.weibosdk_send_failed, 
							Toast.LENGTH_SHORT).show();
				}
				...
				public void onComplete(Bundle values) {
					...
					long overTime = System.currentTimeMillis() + Integer.valueOf(expires_in);
					if(StringUtils.DEBUG){
						Log.i(TAG, "授权成功");
						Log.i(TAG, "token:" + token);
						Log.i(TAG, "expires_in:" + expires_in);
						Log.i(TAG, "overTime:" + DateFormat.format("yyyy-MM-dd hh:mm:ss", overTime));
					}
					ShareTool.getInstance().setAccessToken(token);
					ShareTool.getInstance().setExpiresIn(expires_in);
					ShareTool.getInstance().setOvertime(overTime);
					shareToWeibo();
				}
	...
	private void startPay(int waresid, int price,String exorderno) {
		...
		//payRequest.addParam("notifyurl", UrlUtils.URL_PAYMENT_CALLBACK);
...
//		payRequest.addParam("appuserid", appuserid);
		String paramUrl = payRequest.genSignedUrlParamString(IAppPaySDKConfig.APP_KEY);
		SDKApi.startPay(GameActivity.this, paramUrl, new IPayResultCallback() {
			@Override
			public void onPayResult(int resultCode, String signValue,
				...
				if (SDKApi.PAY_SUCCESS == resultCode) {
					...
					if (null == signValue) {
						...
						Toast.makeText(GameActivity.this, "没有签名值", Toast.LENGTH_SHORT)
								.show();
						// //finish();
					}
					
					Log.e("yyy", signValue + " ");
					boolean flag = PayRequest.isLegalSign(signValue,IAppPaySDKConfig.APP_KEY);
					if (flag) {
						Log.e("payexample", "islegalsign: true");
						Toast.makeText(GameActivity.this, "支付成功", Toast.LENGTH_SHORT)
								.show();
						// 合法签名值，支付成功，请添加支付成功后的业务逻辑
					} else {
						Toast.makeText(GameActivity.this, "支付成功，但是验证签名失败",
								Toast.LENGTH_SHORT).show();
						// 非法签名值，默认采用finish()，请根据需要修改
					}
				} else if(SDKApi.PAY_CANCEL == resultCode){
					Toast.makeText(GameActivity.this, "取消支付", Toast.LENGTH_SHORT)
					.show();
					// 取消支付处理，默认采用finish()，请根据需要修改
					Log.e("fang", "return cancel");
				}else {
					Toast.makeText(GameActivity.this, "支付失败", Toast.LENGTH_SHORT)
							.show();
...
//					// TODO Auto-generated method stub
//					if(currtentUrl.equals(UrlUtils.URL_GET_GEMS)){
...
//				
//				@Override
//				public void transactionFail(boolean propmt) {
//					// TODO Auto-generated method stub
//					if(pd != null){
//						if(pd.isShowing()){
//							pd.dismiss();
...
//						pd = null;
//					}
//					if(currtentUrl.equals(UrlUtils.URL_GET_GEMS) && propmt){
//						UserDefinedDialog.getInstance().confirmDialog(
//								GameActivity.this,R.drawable.app_icon2,
//								GameActivity.this.getString(R.string.app_name), 
//								GameActivity.this.getString(R.string.check_exception), null).show();
//					}
//					handlerBuy = false;
//				}
//				
//				@Override
//				public void checkStatusCodeError(int code) {
//					// TODO Auto-generated method stub
//					if(pd != null){
//						if(pd.isShowing()){
//							pd.dismiss();
...
//								GameActivity.this,R.drawable.app_icon2,
//								GameActivity.this.getString(R.string.app_name), 
//								GameActivity.this.getString(R.string.check_con_fail) + code, null).show();
//					}
//					handlerBuy = false;
//				}
//				
...
//								GameActivity.this,R.drawable.app_icon2,
//								GameActivity.this.getString(R.string.app_name), 
//								GameActivity.this.getString(R.string.check_fail), null).show();
//					}
//					handlerBuy = false;
//				}
//				
...
//								GameActivity.this,R.drawable.app_icon2,
//								GameActivity.this.getString(R.string.app_name), 
//								GameActivity.this.getString(R.string.check_error), null).show();
//					}
//					handlerBuy = false;
//				}
//				
//				@Override
//				public void checkDone(String tState) {
//					// TODO Auto-generated method stub
//					if(pd != null){
//						if(pd.isShowing()){
//							pd.dismiss();
...
//					}
//					handlerBuy = false;
//					if(tState.equals("done")){
//						Log.e(TAG, "currtenUrl:"+currtentUrl);
//						MobageSDKTool.getInstance().saveTid(GameActivity.this,null);
//						if(currtentUrl.equals(UrlUtils.URL_GET_GEMS)){
//							Toast.makeText(GameActivity.this, R.string.check_done, Toast.LENGTH_SHORT).show();
//							webView.reload();
//						}
//					}
//					else if(tState.equals("oauth_null") || tState.equals("data_null")){
//						if(currtentUrl.equals(UrlUtils.URL_GET_GEMS)){
//							Toast.makeText(GameActivity.this, R.string.check_fail, Toast.LENGTH_SHORT).show();
//						}
//					}
//					else{
//						MobageSDKTool.getInstance().saveTid(GameActivity.this,null);
	...
	class MyWebViewClient extends WebViewClient{
		...
		private void loadingDone(String url){
			...
			if(url.equals(UrlUtils.URL_REWARD_CARD) || url.equals(UrlUtils.URL_REWARD_ITEM)
				...
				if(shareType == ShareModel.SHARE_TYPE_GETCARD){
					webView.loadUrl(UrlUtils.URL_CALL);
				}
				else if(shareType == ShareModel.SHARE_TYPE_UP){
					webView.goBack();
				}
				else{
					webView.goBack();
